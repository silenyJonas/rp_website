...
use Illuminate\Http\Request;
use Illuminate\Support\Facades\Log;
use App\Http\Requests\StoreRawRequestCommissionRequest; // Importujte Form Request
...
class RawRequestCommissionController extends Controller
{
    ...
    {
        // Validace se provede AUTOMATICKY předtím, než se kód dostane sem.
        $validatedData = $request->validated();

        // Nastavíme výchozí hodnoty pro status a priority, které nejsou posílány z frontendu.
        // Ujistěte se, že tyto řetězce přesně odpovídají povoleným hodnotám ve vaší databázi/modelu.
        $validatedData['status'] = 'Nově zadané'; // Výchozí stav pro nové požadavky
        $validatedData['priority'] = 'Nízká';     // Výchozí priorita
    ...
    public function update(Request $request, RawRequestCommission $rawRequestCommission)
    {
        // Pro update metodu můžete stále používat `sometimes` pro status a priority,
        // protože je administrátor může měnit.
        $validatedData = $request->validate([
            'thema' => 'sometimes|required|string|max:255',
            'contact_email' => 'sometimes|required|email|max:255',
            'contact_phone' => 'nullable|string|max:255',
            'order_description' => 'sometimes|required|string|max:255', // Zde jsem přidal max:255 pro konzistenci
            'status' => 'sometimes|required|string|in:Nově zadané,Zpracovává se,Dokončeno,Zrušeno',
            'priority' => 'sometimes|required|string|in:Nízká,Neutrální,Vysoká',
        ]);
