...
@Component({
  ...
  templateUrl: './faq.component.html',
  styleUrls: ['./faq.component.css'],
  // Přidáváme OnPush pro lepší výkon a kontrolu nad překreslením
...
export class FaqComponent implements OnInit, OnDestroy {
  ...
  // Do konstruktoru přidáváme ChangeDetectorRef
  constructor(
  ...
  ngOnInit(): void {
    // Odebíráme změny z localizationService
    this.localizationService.currentTranslations$
      .pipe(takeUntil(this.destroy$))
      .subscribe(translations => {
        // Podmínka translations && Object.keys(translations).length > 0 zajistí, 
        // že máme reálná data předtím, než začneme mapovat pole
        if (translations && Object.keys(translations).length > 0) {
          
          // 1. Překlad statických textů
          this.header_1_text = this.localizationService.getText('faq.header_1');
          this.write_us_prompt_text = this.localizationService.getText('faq.write_us_prompt');

          // 2. Naplnění pole FAQ položek
          this.loadFaqItems();

          // 3. KLÍČOVÝ KROK: Oznámení Angularu, že došlo ke změnám
          // Bez tohoto řádku se může stát, že HTML zůstane v češtině, i když TS má angličtinu (nebo naopak)
  ...
  private loadFaqItems(): void {
    // Vytvoříme nové pole (imutabilita), aby si Angular všiml změny reference
  ...
  toggleFaq(item: FaqItem): void {
    item.isActive = !item.isActive;
    // Jelikož používáme OnPush, musíme detectChanges zavolat i při interakci (kliknutí),
    // aby se projevilo rozbalení/sbalení (isActive)
    this.cdr.detectChanges();
  }
