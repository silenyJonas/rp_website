...
import { GenericFormComponent } from '../../components/generic-form/generic-form.component'; // Cesta k GenericFormComponent
import { FormFieldConfig } from '../../../shared/interfaces/form-field-config'; // Cesta k rozhraní FormFieldConfig
import { FormsModule } from '@angular/forms'; // FormsModule je potřeba pro komponentu, která GenericForm používá
import { PublicDataService } from '../../services/public-data.service'; // Cesta k PublicDataService
...
export class ServicesComponent implements OnInit {
  ...
  constructor(private publicDataService: PublicDataService, private cdr: ChangeDetectorRef) { } // cdr je již v konstruktoru
  ...
  handleFormSubmission(formData: any): void {
    ...
    this.publicDataService.submitContactForm(formData).subscribe({
      next: (response) => {
        ...
        // Zde můžete přidat globální notifikaci, pokud GenericFormComponent
        // nezobrazuje úspěšnou zprávu sám, nebo pokud chcete něco navíc.
      },
      error: (error: HttpErrorResponse) => {
        console.error('Chyba při odesílání formuláře přes PublicDataService:', error);
        // Zde můžete přidat globální notifikaci o chybě.
      }
    });
  }
  ...
  // Ostatní data a metody pro služby a technologie
  webServices: Item[] = [
